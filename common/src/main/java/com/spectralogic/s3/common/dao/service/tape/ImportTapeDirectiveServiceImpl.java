...
final class ImportTapeDirectiveServiceImpl 
    extends BaseService< ImportTapeDirective > implements ImportTapeDirectiveService
{
    ImportTapeDirectiveServiceImpl()
    {
        super( ImportTapeDirective.class );
    }
    
    
    public ImportTapeDirective attainByEntityToImport( final UUID idOfEntityToImport )
    {
    ...
    public void deleteAll()
    {
        if ( 0 < getServiceManager().getRetriever( Tape.class ).getCount( Require.beanPropertyEqualsOneOf( 
                Tape.STATE, TapeState.IMPORT_IN_PROGRESS, TapeState.IMPORT_PENDING ) ) )
        {
