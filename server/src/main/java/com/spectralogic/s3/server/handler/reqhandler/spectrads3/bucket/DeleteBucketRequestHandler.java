...
public final class DeleteBucketRequestHandler extends BaseRequestHandler
{
    ...
    {
        final Bucket bucket = request.getRestRequest()
                .getBean( params.getServiceManager().getRetriever( Bucket.class ) );
        ...
        final boolean force = request.hasRequestParameter( RequestParameterType.FORCE );
        
        params.getTargetResource().deleteBucket( 
                request.getAuthorization().getUserId(),
                bucket.getId(),
                force ).get( Timeout.LONG );
