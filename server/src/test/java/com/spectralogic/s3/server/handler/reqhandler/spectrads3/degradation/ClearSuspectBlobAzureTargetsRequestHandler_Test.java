...
public final class ClearSuspectBlobAzureTargetsRequestHandler_Test
{
    @Test
    public void testClearAllDoesSoIfForceFlagUsed()
    {
        ...
        mockDaoDriver.makeSuspect( mockDaoDriver.putBlobOnAzureTarget( target1.getId(), b1.getId() ) );
        ...
        final StorageDomainMember sdm1 =
                mockDaoDriver.addTapePartitionToStorageDomain( sd.getId(), tape1.getPartitionId(), TapeType.LTO5 );
        ...
        assertEquals(0,  support.getDatabaseSupport().getServiceManager().getRetriever(
                SuspectBlobAzureTarget.class).getCount(), "Shoulda deleted all suspect records of the appropriate type.");
        assertEquals(0,  support.getDatabaseSupport().getServiceManager().getRetriever(
    ...
    public void testClearSpecificIdsDoesSo() throws UnsupportedEncodingException
    {
        ...
        final S3Object obj1 = mockDaoDriver.createObject( null, "o1" );
        final Blob blobObj1 = mockDaoDriver.getBlobFor( obj1.getId() );
        final BlobAzureTarget bTarget = mockDaoDriver.makeSuspect( 
                mockDaoDriver.putBlobOnAzureTarget( target1.getId(), blobObj1.getId() ) );
        
        mockDaoDriver.makeSuspect( mockDaoDriver.putBlobOnTape( tape2.getId(), blobObj1.getId() ) );
        mockDaoDriver.makeSuspect( mockDaoDriver.putBlobOnPool( pool2.getId(), blobObj1.getId() ) );
        mockDaoDriver.makeSuspect( mockDaoDriver.putBlobOnAzureTarget( target2.getId(), blobObj1.getId() ) );
        ...
        driver.setRequestPayload( 
                ( "<ids><id>" + bTarget.getId() + "</id></ids>" ).getBytes( "UTF8" ) );
        driver.run();
        driver.assertHttpResponseCodeEquals( 204 );
        assertEquals(1,  support.getDatabaseSupport().getServiceManager().getRetriever(
                SuspectBlobTape.class).getCount(), "Shoulda deleted only 1 suspect records of the appropriate type.");
        assertEquals(1,  support.getDatabaseSupport().getServiceManager().getRetriever(
                SuspectBlobPool.class).getCount(), "Shoulda deleted only 1 suspect records of the appropriate type.");
        assertEquals(1,  support.getDatabaseSupport().getServiceManager().getRetriever(
                SuspectBlobAzureTarget.class).getCount(), "Shoulda deleted specified suspect records of the appropriate type.");
        assertEquals(0,  support.getDatabaseSupport().getServiceManager().getRetriever(
                DegradedBlob.class).getCount(), "Should notta recorded any degradation.");
    }
    ...
    public void testClearSpecificIdsThrows400WithIdCountMismatch() throws UnsupportedEncodingException
    {
        ...
        assertEquals(2,  support.getDatabaseSupport().getServiceManager().getRetriever(
                SuspectBlobAzureTarget.class).getCount(), "Shoulda NOT deleted specified suspect records of the appropriate type.");
        assertEquals(0,  support.getDatabaseSupport().getServiceManager().getRetriever(
                DegradedBlob.class).getCount(), "Should notta recorded any degradation.");
    }
}
