...
public final class EjectStorageDomainRequestHandler_Test 
{
    @Test
    public void testtestEjectBucketCallsDataPlanner()
    {
        final MockHttpRequestSupport support = new MockHttpRequestSupport();
        final MockDaoDriver mockDaoDriver = new MockDaoDriver( support.getDatabaseSupport() );

        final Method method = ReflectUtil.getMethod( TapeManagementResource.class, "ejectStorageDomain" );
        final MediaOperationInvocationHandler toih = new MediaOperationInvocationHandler();
        final BasicTestsInvocationHandler btih = new BasicTestsInvocationHandler( toih );
        ...
        assertEquals(null, btih.getMethodInvokeData( method ).get( 0 ).getArgs().get( 1 ), "Shoulda sent only the expected bucket id.");
        ...
        assertEquals(null, btih.getMethodInvokeData( method ).get( 0 ).getArgs().get( 3 ), "Shoulda sent down null for non-specified eject tape attributes.");
        assertEquals(0,  Array.getLength(btih.getMethodInvokeData(method).get(0).getArgs().get(4)), "Should notta sent down any blobs.");
        ...
        assertEquals(1,  btih.getMethodCallCount(method), "Shoulda sent only the expected storageDomain id.");
        assertEquals(storageDomainId, btih.getMethodInvokeData( method ).get( 0 ).getArgs().get( 0 ), "Shoulda sent only the expected storageDomain id.");
        assertEquals(bucketId, btih.getMethodInvokeData( method ).get( 0 ).getArgs().get( 1 ), "Shoulda sent only the expected bucket id.");
        assertEquals(null, btih.getMethodInvokeData( method ).get( 0 ).getArgs().get( 2 ), "Shoulda sent down null for non-specified eject tape attributes.");
        assertEquals(null, btih.getMethodInvokeData( method ).get( 0 ).getArgs().get( 3 ), "Shoulda sent down null for non-specified eject tape attributes.");
        assertEquals(0,  Array.getLength(btih.getMethodInvokeData(method).get(0).getArgs().get(4)), "Should notta sent down any blobs.");
        ...
        btih.reset();
        toih.clear();
        toih.addTapeFailure( UUID.randomUUID(), "oops" );
        driver = new MockHttpRequestDriver( 
                support,
                true,
                new MockInternalRequestAuthorizationStrategy(),
    ...
    @Test
    public void testtestEjectBucketAllOptionalAttributesSpecifiedCallsDataPlanner()
    {
        final MockHttpRequestSupport support = new MockHttpRequestSupport();
        final MockDaoDriver mockDaoDriver = new MockDaoDriver( support.getDatabaseSupport() );
