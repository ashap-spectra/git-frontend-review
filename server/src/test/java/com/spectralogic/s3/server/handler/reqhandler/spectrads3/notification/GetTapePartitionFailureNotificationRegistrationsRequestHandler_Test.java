...
public final class GetTapePartitionFailureNotificationRegistrationsRequestHandler_Test 
{
    @Test
    public void testGetTapePartitionFailureNotificationRegistrationsReturnsRegistrations()
    {
        final MockHttpRequestSupport support = new MockHttpRequestSupport();

        final MockDaoDriver mockDaoDriver = new MockDaoDriver( support.getDatabaseSupport() );
        final UUID userId = mockDaoDriver.createUser( MockDaoDriver.DEFAULT_USER_NAME ).getId();
        final List< TapePartitionFailureNotificationRegistration > registrations =
        ...
        final MockHttpRequestDriver driver = new MockHttpRequestDriver( 
                ...
                RequestType.GET, 
                "_rest_/" + RestDomainType.TAPE_PARTITION_FAILURE_NOTIFICATION_REGISTRATION.toString() );
        driver.run();
        driver.assertHttpResponseCodeEquals( 200 );

        new HttpResponseContentVerifier( driver )
                .verifyNotificationRegistrationNode(
                        "/Data/TapePartitionFailureNotificationRegistration[1]/",
                        registrations.get( 0 ) )
                .verifyNotificationRegistrationNode(
                        "/Data/TapePartitionFailureNotificationRegistration[2]/",
                        registrations.get( 1 ) );
    }
}
