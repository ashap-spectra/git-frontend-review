...
public final class GetObjectLostNotificationRegistrationsRequestHandler_Test 
{
    @Test
    public void testGetObjectLostNotificationRegistrationsReturnsRegistrations()
    {
        final MockHttpRequestSupport support = new MockHttpRequestSupport();

        final MockDaoDriver mockDaoDriver = new MockDaoDriver( support.getDatabaseSupport() );
        final UUID userId = mockDaoDriver.createUser( MockDaoDriver.DEFAULT_USER_NAME ).getId();
        final List< S3ObjectLostNotificationRegistration > registrations =
        ...
        new HttpResponseContentVerifier( driver )
                .verifyNotificationRegistrationNode(
                        "/Data/S3ObjectLostNotificationRegistration[1]/",
                        registrations.get( 0 ) )
                .verifyNotificationRegistrationNode(
                        "/Data/S3ObjectLostNotificationRegistration[2]/",
                        registrations.get( 1 ) );
    }
}
