...
public final class GetDs3TargetFailureNotificationRegistrationsRequestHandler_Test 
{
    @Test
    public void testGetDs3TargetFailureNotificationRegistrationsReturnsRegistrations()
    {
        final MockHttpRequestSupport support = new MockHttpRequestSupport();

        final MockDaoDriver mockDaoDriver = new MockDaoDriver( support.getDatabaseSupport() );
        final UUID userId = mockDaoDriver.createUser( MockDaoDriver.DEFAULT_USER_NAME ).getId();
        final List< Ds3TargetFailureNotificationRegistration > registrations =
        ...
        final MockHttpRequestDriver driver = new MockHttpRequestDriver( 
                ...
                RequestType.GET, 
                "_rest_/" + RestDomainType.DS3_TARGET_FAILURE_NOTIFICATION_REGISTRATION.toString() );
        driver.run();
        driver.assertHttpResponseCodeEquals( 200 );

        new HttpResponseContentVerifier( driver )
                .verifyNotificationRegistrationNode(
                        "/Data/Ds3TargetFailureNotificationRegistration[1]/",
                        registrations.get( 0 ) )
                .verifyNotificationRegistrationNode(
                        "/Data/Ds3TargetFailureNotificationRegistration[2]/",
                        registrations.get( 1 ) );
    }
}
