...
public final class GetTapeFailureNotificationRegistrationsRequestHandler_Test 
{
    @Test
    public void testGetTapeFailureNotificationRegistrationsReturnsRegistrations()
    {
        final MockHttpRequestSupport support = new MockHttpRequestSupport();

        final MockDaoDriver mockDaoDriver = new MockDaoDriver( support.getDatabaseSupport() );
        final UUID userId = mockDaoDriver.createUser( MockDaoDriver.DEFAULT_USER_NAME ).getId();
        final List< TapeFailureNotificationRegistration > registrations =
        ...
        final MockHttpRequestDriver driver = new MockHttpRequestDriver( 
                ...
                RequestType.GET, 
                "_rest_/" + RestDomainType.TAPE_FAILURE_NOTIFICATION_REGISTRATION.toString() );
        driver.run();
        driver.assertHttpResponseCodeEquals( 200 );

        new HttpResponseContentVerifier( driver )
                .verifyNotificationRegistrationNode(
                        "/Data/TapeFailureNotificationRegistration[1]/",
                        registrations.get( 0 ) )
                .verifyNotificationRegistrationNode(
                        "/Data/TapeFailureNotificationRegistration[2]/",
                        registrations.get( 1 ) );
    }
}
