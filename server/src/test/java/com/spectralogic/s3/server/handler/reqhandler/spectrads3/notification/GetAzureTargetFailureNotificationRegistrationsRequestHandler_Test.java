...
public final class GetAzureTargetFailureNotificationRegistrationsRequestHandler_Test 
{
    @Test
    public void testGetAzureTargetFailureNotificationRegistrationsReturnsRegistrations()
    {
        final MockHttpRequestSupport support = new MockHttpRequestSupport();

        final MockDaoDriver mockDaoDriver = new MockDaoDriver( support.getDatabaseSupport() );
        final UUID userId = mockDaoDriver.createUser( MockDaoDriver.DEFAULT_USER_NAME ).getId();
        final List< AzureTargetFailureNotificationRegistration > registrations =
        ...
        final MockHttpRequestDriver driver = new MockHttpRequestDriver( 
                ...
                RequestType.GET, 
                "_rest_/" + RestDomainType.AZURE_TARGET_FAILURE_NOTIFICATION_REGISTRATION.toString() );
        driver.run();
        driver.assertHttpResponseCodeEquals( 200 );

        new HttpResponseContentVerifier( driver )
                .verifyNotificationRegistrationNode(
                        "/Data/AzureTargetFailureNotificationRegistration[1]/",
                        registrations.get( 0 ) )
                .verifyNotificationRegistrationNode(
                        "/Data/AzureTargetFailureNotificationRegistration[2]/",
                        registrations.get( 1 ) );
    }
}
