...
public class VerifyJobTapeTest {
    private Ds3Client client;
    final String bucketName = "single_copy_bucket";
    String inputPath = "testFiles";
    private final static Logger LOG = Logger.getLogger( VerifyJobTapeTest.class );
    ...
    public void testPutJobToTape() throws IOException, InterruptedException {
        LOG.info("Starting test : VerifyJobTest" );
        try {
            ...
            addJobName(client, "VerifyJobTest", currentJobId);
            System.out.println("Waiting for PUT job to complete ----");

            isJobCompleted(client, currentJobId);
            System.out.println("PUT job completed. Going to clear cache ----");
            reclaimCache(client);
