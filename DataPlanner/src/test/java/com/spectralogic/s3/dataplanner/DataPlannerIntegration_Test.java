...
public final class DataPlannerIntegration_Test {
    ...
    @Test
    public void testPutJobToPermAndTempCopyOnPool() {
        // Two pool copies driven by one PERMANENT rule and one TEMPORARY rule on different
        // storage domains. Each copy must be written by its own WriteChunkToPoolTask, so this
        // exercises the recent change that splits a single multi-destination directive into
        // one directive per storage domain.
        try (final DataPlannerIntegrationTester tester = new DataPlannerIntegrationTester(
                Simulator.getTestConfig())) {
            tester.start();
            ...
            final DataPolicy dp = mockDaoDriver.createABMConfigPermAndTempCopyOnPool();
            ...
            final CreatePutJobParams params = BeanFactory.newBean(CreatePutJobParams.class)
                    ...
                    .setObjectsToCreate(new S3ObjectToCreate[] { BeanFactory.newBean(S3ObjectToCreate.class)
                            .setName("o1").setSizeInBytes(10) });
            final UUID jobId = plannerResource.createPutJob(params).get(RpcFuture.Timeout.DEFAULT);
            assertEquals(2, serviceManager.getRetriever(LocalBlobDestination.class).getCount(),
                    "There should be one destination per storage domain (perm + temp)");
            final UUID blobId = serviceManager.getService(BlobService.class).attain(Require.nothing()).getId();
            final UUID chunkId = mockDaoDriver.attainOneAndOnly(JobEntry.class).getId();
            plannerResource.allocateEntry(chunkId).get(RpcFuture.Timeout.LONG);
            BlobCache bc = mockDaoDriver.attainOneAndOnly(BlobCache.class);
            assertEquals(CacheEntryState.ALLOCATED, bc.getState(),
                    "Blob should be allocated, but not yet in cache");
            plannerResource.startBlobWrite(jobId, blobId).get(RpcFuture.Timeout.DEFAULT);
            cacheFilesystemDriver.writeCacheFile(blobId, 10);
            plannerResource.blobWriteCompleted(jobId, blobId, ChecksumType.MD5, "TEST_CHECKSUM", null,
                    new S3ObjectProperty[]{}).get(RpcFuture.Timeout.DEFAULT);
            bc = mockDaoDriver.attainOneAndOnly(BlobCache.class);
            assertEquals(CacheEntryState.IN_CACHE, bc.getState(), "Blob should be in cache now");
            ...
            while (serviceManager.getService(JobService.class).any(Require.nothing())) {
                if (System.currentTimeMillis() - startTime > 5 * 60 * 1000) {
                    throw new RuntimeException("Timed out after 5 minutes waiting for perm+temp pool job to complete");
                }
                TestUtil.sleep(1000);
            }
            assertEquals(1, serviceManager.getService(CompletedJobService.class).getCount(),
                    "There should be one completed job");
            assertEquals(0, serviceManager.getRetriever(LocalBlobDestination.class).getCount(),
                    "Both perm and temp destinations should be cleared once the job completes");
            assertEquals(2, serviceManager.getService(BlobPoolService.class).getCount(),
                    "There should be two blob pool records, one for the perm copy and one for the temp copy");
        }
    }
